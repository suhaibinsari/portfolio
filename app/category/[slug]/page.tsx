...
export default function Page({ params }: { params: { slug: string } }) {
  const result = getProjectsByCategory(params.slug)

  return <div className='py-20 px-2 mx-auto' id="section0">
            ...
            <Link href={project.link} target="_blank" key={index}>
              <div className="relative isolate flex flex-col justify-end overflow-hidden rounded-2xl h-[20rem] w-auto group px-6 py-3">
                ...
                <div className='flex items-center justify-center'>
                  ...
                  <p className="z-10 text-gray-700 hidden ">{project.category}</p>
                </div>
                <Image
                  src={project.image}
                  alt={project.title}
                  layout="fill"
                  objectFit="cover"
                  className="absolute inset-0 object-top transition duration-300 transform scale-100 group-hover:scale-105"
                />
              </div>
            </Link>
          ))}
